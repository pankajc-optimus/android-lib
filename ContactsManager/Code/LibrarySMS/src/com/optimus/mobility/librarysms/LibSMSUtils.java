...
public class LibSMSUtils {
	...
	public JSONArray getTextsfromId(Context argContext, String conversationId)
		...
		try {
			...
			throw new JSONException("Unable to parse into JSON");
		}
		// Return the result obtained.
		return obj;
	}
	 ...
	 *            Context of the activity calling this function.
	 * @return JSONArray which consists of JSONObjects.The fields represent
	 *         MessageTextId,Status,Senders Address,TextMessage,Date with key values
	 *         "messageid","status"(STATUS_NONE = -1;STATUS_COMPLETE =
	...
	public JSONArray getContacts(Context argContext) throws JSONException {
		...
		final String[] projection = new String[] { Data._ID, Phone.NUMBER,
				Phone.DATA, Phone.DATA1, Phone.TYPE, Phone.LABEL,
	...
	public JSONArray getSMSLog(Context argContext, String timeStamp)
		...
		if (timeString.length() == 10) {
			timeString = timeString.append("000");
			Log.d("TimeStamp", "" + timeString.toString());
		} else if (timeString.length() == 13) {

		} else {
			throw new IllegalArgumentException(
					"Illegal TimeStamp Value Entered.");
		}
